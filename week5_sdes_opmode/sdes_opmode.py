...
def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    ...
    for i in range(ciphertext.__len__()//8):
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv: bitarray):
    ...
    for i in range(text.__len__()//8):
        if i == 0:
            use_iv = text[i*8:(i+1)*8] ^ iv
            result += sdes(use_iv, key, MODE_ENCRYPT)
        else:
            pre_bit = text[i*8:(i+1)*8] ^ result[(i-1)*8:i*8]
            result += sdes(pre_bit, key, MODE_ENCRYPT)
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv: bitarray):
    ...
    count = int(ciphertext.__len__() / 8)
    first_ciphertext = ciphertext[0:8]
    first_sdes = sdes(first_ciphertext, key, MODE_DECRYPT)
    result += first_sdes ^ iv

    for i in range(1, count):
        use_sdes = sdes(ciphertext[i*8:(i+1)*8], key, MODE_DECRYPT)
        result += use_sdes ^ result[(i - 1) * 8:i * 8]
