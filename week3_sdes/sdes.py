...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    decrypt_result = bitarray()
    ip_bits = bitarray()

    # Place your own implementation of S-DES Here
    if mode == MODE_ENCRYPT:
        #Initial Permutation pass
        for i in IP:
        ...
        #Generate round_key
        round_key = schedule_keys(key)

        round_result = round(ip_bits_r, round_key[0])
        result = ip_bits_l ^ round_result

        bits_l = ip_bits_r
        bits_r = result

        round_result_2 = round(bits_r, round_key[1])
        result = bits_l ^ round_result_2

        result = result + bits_r
        for i in IP_1:
            encrypt_result.append(result[i])
        ...
        round_result = round(ip_bits_r, round_key[1])
        result = ip_bits_l ^ round_result

        bits_l = ip_bits_r
        bits_r = result

        round_result_2 = round(bits_r, round_key[0])
        result = bits_l ^ round_result_2

        result = result + bits_r
        for i in IP_1:
            decrypt_result.append(result[i])
